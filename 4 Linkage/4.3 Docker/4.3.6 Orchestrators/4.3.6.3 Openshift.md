...
Deployments используется для **Stateless-приложение** (приложение без сохраняемого состояния), которое не помнит ничего между запросами. Каждый запрос обрабатывается независимо. Перезапустить его, ничего страшного не случится. Примеры: веб-серверы Nginx, бэкенд-API на Node.js/Python/Go, которые хранят состояние во внешней базе данных.
